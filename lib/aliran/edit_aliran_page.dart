...
class EditAliranPage extends StatefulWidget {
  ...
  const EditAliranPage({Key? key, required this.aliran}) : super(key: key);

  @override
  _EditAliranPageState createState() => _EditAliranPageState();
}

class _EditAliranPageState extends State<EditAliranPage> {
  ...
  void initState() {
    super.initState();
    _laporanPengaduanController.text = widget.aliran.laporan_pengaduan ?? '';
    _namaPelaporController.text = widget.aliran.nama_pelapor ?? '';
    _ktpController.text = widget.aliran.ktp ?? '';
    _noHpController.text = widget.aliran.no_hp ?? '';
  }
  ...
  Future<void> _updateAliran() async {
    if (_formKey.currentState!.validate()) {
      ...
      try {
        Uri uri = Uri.parse('http://192.168.1.8/kejaksaan/editpengawasan.php');

        http.MultipartRequest request = http.MultipartRequest('POST', uri)
          ..fields['id'] = widget.aliran.id
          ..fields['user_id'] = widget.aliran.user_id
          ...
          ..fields['status'] = widget.aliran.status;
