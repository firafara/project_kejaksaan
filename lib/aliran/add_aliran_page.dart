...
class AddAliranPage extends StatefulWidget {
  ...
  @override
  State<AddAliranPage> createState() => _AddAliranPageState();
}

class _AddAliranPageState extends State<AddAliranPage> {
  ...
  Future<void> addPengaduan() async {
    ...
    try {
      Uri uri = Uri.parse('http://192.168.31.53/kejaksaan/addpengawasan.php');
      ...
      if (response.statusCode == 200) {
        try {
          ModelAddAliran data = modelAddAliranFromJson(responseBody);
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text('${data.message}')),
          );
          if (data.isSuccess) {
