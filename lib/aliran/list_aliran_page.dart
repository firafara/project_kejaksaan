...
class _ListAliranPageState extends State<ListAliranPage> {
  ...
  Future<void> _fetchAliran() async {
    final response = await http.get(Uri.parse('http://192.168.1.3/kejaksaan/pengawasan.php'));
    if (response.statusCode == 200) {
      final parsed = jsonDecode(response.body);
      if (parsed['data'] != null) {
        setState(() {
  ...
  Future<void> _saveStatus(Datum aliran, String status) async {
    try {
      final response = await http.post(
        Uri.parse('http://192.168.1.3/kejaksaan/updateStatusPengawasan.php'),
        body: {
          'id': aliran.id,
          'status': status,
        },
  ...
  Future<void> _handleDelete(Datum aliran) async {
    try {
      final response = await http.post(
        Uri.parse('http://192.168.1.3/kejaksaan/deletepengawasan.php'), // Sesuaikan dengan URL endpoint untuk hapus data
        body: {
          'id': aliran.id,
        },
      );
