...
import 'package:project_kejaksaan/models/model_add_pilkada.dart';
...
class AddPilkadaPage extends StatefulWidget {
  ...
  @override
  State<AddPilkadaPage> createState() => _AddPilkadaPageState();
}
...
class _AddPilkadaPageState extends State<AddPilkadaPage> {
  ...
  Future<void> addPengaduan() async {
    ...
    try {
      Uri uri = Uri.parse('http://192.168.31.53/kejaksaan/addpilkada.php');
      ...
      if (response.statusCode == 200) {
        try {
          ModelAddPilkada data = modelAddPilkadaFromJson(responseBody);
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text('${data.message}')),
          );
          if (data.isSuccess) {
