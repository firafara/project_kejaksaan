...
class _ListPilkadaPageState extends State<ListPilkadaPage> {
  ...
  Future<void> _fetchPilkada() async {
    final response = await http.get(Uri.parse('http://192.168.31.53/kejaksaan/pilkada.php'));
    if (response.statusCode == 200) {
      final parsed = jsonDecode(response.body);
      if (parsed['data'] != null) {
        setState(() {
  ...
  Future<void> _saveStatus(Datum pilkada, String status) async {
    try {
      final response = await http.post(
        Uri.parse('http://192.168.31.53/kejaksaan/updateStatusPilkada.php'),
        body: {
          'id': pilkada.id,
          'status': status,
        },
  ...
  Future<void> _handleDelete(Datum pilkada) async {
    try {
      final response = await http.post(
        Uri.parse('http://192.168.31.53/kejaksaan/deletepilkada.php'), // Sesuaikan dengan URL endpoint untuk hapus data
        body: {
          'id': pilkada.id,
        },
      );
